...
def process_refund():
    ...
    # Required Inputs from the Frontend/Postman
    sale_id = data.get('sale_id')
    sale_detail_id = data.get('sale_detail_id') 
    qty_to_return = int(data.get('quantity', 0))
    reason = data.get('reason')
    
    # 1. VALIDATION: Check for missing data and valid reason
    ...
    cur = mysql.connection.cursor()
    try:
        # 2. VERIFY ITEM EXISTS & CHECK PREVIOUS REFUNDS
        # We sum up any existing returns in RETURN_ITEMS for this specific sale_detail_id
        cur.execute("""
        ...
        if not original_item:
            return jsonify({"message": "Verification Failed: This item was not found on Receipt #" + str(sale_id)}), 404
        ...
        # Calculate remaining quantity that can actually be refunded
        ...
        # Calculate Net Price per item (Price minus the discount they got)
        ...
        # 4. RECORD THE REFUND (Header & Item Detail)
        cur.execute("""
        ...
        # 5. RESTORE STOCK & LOG ADJUSTMENT
        ...
        """, (inv_id, manager_id, qty_to_return, f"Refunded Item from Sale #{sale_id}. Reason: {reason}"))
